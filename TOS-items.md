...
| C6 | **Free-tier = recurring monthly credit grant (decided).** Not a one-time seed: signed-in users receive a generous monthly top-up (a loss-leader), applied idempotently per calendar month (`credits_period` = YYYY-MM). The `@default(10)` balance is a cold-start seed only. Unused credits do **not** roll over beyond the monthly top-up logic. | Sets the exact free-tier promise; avoids implying unlimited or implying accrual/rollover. | API docs, billing UI, ToS | planned (decision locked; grant code live, search-spend pending) |
