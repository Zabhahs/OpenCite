...
- **Free-tier decision locked:** generous **recurring monthly** top-up as a loss-leader (the `@default(10)` seed is cold-start only). Implemented via `applyMonthlyGrant` keyed on `credits_period` (YYYY-MM) for idempotency.
