...
**Rule: always preserve all existing dependencies when modifying `package.json`.**

When patching `package.json` for any reason (adding scripts, bumping versions, adding new packages), the full file must be reconstructed from the last known-good state — never from the original repo file if Phase 1+ deps have already been added. Failure to do this silently drops packages, causing build failures that are non-obvious to diagnose.

Current dependency baseline (post Phase 1):
...
Before adding any new package, read the current `package.json` from the repo first — never reconstruct from memory.
