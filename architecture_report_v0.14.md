...
# Auth.js v5 + Supabase + Prisma. Cross-device sync for signed-in users. Anonymous users unaffected. See `v0.14\_phase1\_alpha\_technical\_doc.md` for full detail.
...
# \- \*\*Google OAuth setup required before first sign-in\*\* — Google Cloud Console steps: create project → OAuth consent screen (External/Audience) → create Web Application credentials → register both callback URIs → paste `AUTH\_GOOGLE\_ID` and `AUTH\_GOOGLE\_SECRET` into Vercel dashboard.
...
# │   │   ├── defaults.js                   ← STORAGE\_NS, HISTORY\_MAX, INITIAL\_PAGE\_SIZE,

# │   │   │                                    LOAD\_MORE\_PAGE\_SIZE, REGION\_ORDER,

# │   │   │                                    DEFAULT\_CURATED\_JOURNALS, DEFAULT\_SETTINGS
...
# `useAuth()` returns `{ user, status, signIn, signOut }`. `user.id` is the Supabase `internal\_id` UUID. `BillingContext` reads `user.id` when Phase 3 ships.
...
# \*\*Flat\*\* — baazijan's preference. All component files live directly in `src/components/`, not in subdirectories. Same pattern as `src/hooks/`. Do not reorganise into subdirectories unless explicitly asked.
...
# Current `ALLOWED\_DOMAINS` (10 domains):
...
# \- Stripe webhook → update `users.total\_credits` in Supabase on `invoice.paid`

# \- Upgrade/downgrade UI + Stripe customer portal

# \- Hook column: `users.stripe\_customer\_id` already in schema
...
# \- Hook column: `users.agent\_wallet\_address` already in schema
...
# \- Hook columns: `meta JSONB` on `search\_history` and `library\_items` already in schema
...
# Linked via Vercel marketplace. `DATABASE\_URL` and `DIRECT\_URL` auto-injected into all Vercel environments.
