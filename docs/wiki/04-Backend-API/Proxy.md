...
Note: Six of the nine per-source routes in `api/search/` were created precisely to replace this proxy for sources that needed POST bodies or API keys (see [[04-Backend-API/Per-Source-Routes]]). The proxy handles the remaining GET-based CORS-only sources.
...
[[04-Backend-API/Per-Source-Routes]] · [[09-Audit/Security]] · [[02-Adapters/Adapter-Architecture]]
