...
- **Reuse:** `useHistory`/`useLibrary`/`useSettings` share an identical DB-sync pattern (load from localStorage → syncFromDB on sign-in → write both on mutation). This pattern should be a shared `useSyncedStore` factory — see [[09-Audit/Duplication-and-Reuse#r-300]].

## See also

[[01-Frontend/App-Shell]] · [[01-Frontend/Contexts]] · [[01-Frontend/State-Flow]] · [[03-Search-Pipeline/Semantic-Rerank]] · [[09-Audit/Bugs]]
