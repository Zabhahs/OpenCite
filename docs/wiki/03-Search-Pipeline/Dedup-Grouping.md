...
- **Reuse:** pure JS shared — both client (`hooks.useSearch`) and server (`api.search`) import `dedup.js`. `groupResults.js` is client-only (source-view layout). See [[09-Audit/Duplication-and-Reuse#r-203]].
- **Smells:** No dead code. Both strategies are used. `groupByParentWork` is render-time only (not called during scoring).

## See also

[[Confidence-Gate]] · [[Ranking-Scoring]] · [[02-Adapters/Adapter-Architecture]] · [[09-Audit/Bugs#f-206]]
