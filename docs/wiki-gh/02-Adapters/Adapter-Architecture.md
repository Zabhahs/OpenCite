...
---
<!-- AUTO-GENERATED from docs/wiki/02-Adapters/Adapter-Architecture.md by scripts/wiki/to-github.mjs — do not edit here. Edit the Obsidian source in docs/wiki/ and re-run: node scripts/wiki/to-github.mjs -->
...
The spoof headers are duplicated between `proxy.js:18–24` and `api/proxy.js:69–73`; this is a **documented accepted duplication** because the Edge route and `src/` cannot import each other. See [Duplication-and-Reuse](../09-Audit/Duplication-and-Reuse.md#r-100).
...
- **Findings:**
  - [F-100] `xmlUtils.js` helpers manually duplicated into `api/search/mexicana.js` — constraint is documented but creates a drift risk. See [Duplication-and-Reuse](../09-Audit/Duplication-and-Reuse.md#r-100).
...
- **Reuse:** `parseOpenAlexWork` is already properly shared between OPENALEX and CURATED — good. See [Duplication-and-Reuse](../09-Audit/Duplication-and-Reuse.md#r-101).
- **Smells:**
