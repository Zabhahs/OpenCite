...
---
<!-- AUTO-GENERATED from docs/wiki/03-Search-Pipeline/RRF-Fusion.md by scripts/wiki/to-github.mjs — do not edit here. Edit the Obsidian source in docs/wiki/ and re-run: node scripts/wiki/to-github.mjs -->
...
- Imported by: [Semantic-Rerank](Semantic-Rerank.md) (`hooks.useSemanticRerank` calls `fuseRanks`, `buildNativeRanks`, `nativeWeight`); also available for `api.search` server-side use (not yet called there — see [Duplication-and-Reuse](../09-Audit/Duplication-and-Reuse.md#r-201))
...
- **Reuse:** `lib.rrf` is pure/shared — used by both `hooks.useSemanticRerank` (client, three-input) and `api/search.js` (server, two-input). See [Duplication-and-Reuse](../09-Audit/Duplication-and-Reuse.md#r-201).
...
[Ranking-Scoring](Ranking-Scoring.md) · [Semantic-Rerank](Semantic-Rerank.md) · [Known-Defects](Known-Defects.md#f-200) · [Duplication-and-Reuse](../09-Audit/Duplication-and-Reuse.md#r-201)
