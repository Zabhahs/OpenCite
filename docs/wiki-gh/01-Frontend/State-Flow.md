...
---
<!-- AUTO-GENERATED from docs/wiki/01-Frontend/State-Flow.md by scripts/wiki/to-github.mjs — do not edit here. Edit the Obsidian source in docs/wiki/ and re-run: node scripts/wiki/to-github.mjs -->
...
`SettingsContext` exists but is not used — settings are not in a context. See [Contexts](Contexts.md).
...
- **Reuse:** `useHistory`, `useLibrary`, `useSettings` all implement the same load-from-localStorage → syncFromDB-on-sign-in → write-both-on-mutation pattern. Should be a shared `useSyncedStore(key, apiPath, validate)` factory hook — see [Duplication-and-Reuse](../09-Audit/Duplication-and-Reuse.md#r-300).
