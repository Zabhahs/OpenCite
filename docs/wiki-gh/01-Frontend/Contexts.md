...
---
<!-- AUTO-GENERATED from docs/wiki/01-Frontend/Contexts.md by scripts/wiki/to-github.mjs — do not edit here. Edit the Obsidian source in docs/wiki/ and re-run: node scripts/wiki/to-github.mjs -->
...
All searches pass without any credit check client-side. The real credit metering lives on the server side (`api/_shared/billing.js`, `api/search.js` middleware) — see [Billing-Credits](../05-Billing/Billing-Credits.md) and [Search-Endpoint](../04-Backend-API/Search-Endpoint.md).
...
- **Findings:** [F-300] BillingProvider never mounted (see [Bugs](../09-Audit/Bugs.md#f-300)). [F-308] SettingsContext/SettingsProvider never mounted or consumed (see [Bugs](../09-Audit/Bugs.md#f-308)). [F-311] No client-visible credit balance — users cannot see how many searches they have left; the server gates but the UI is silent.
...
[Hooks](Hooks.md) · [App-Shell](App-Shell.md) · [State-Flow](State-Flow.md) · [Billing-Credits](../05-Billing/Billing-Credits.md) · [Bugs](../09-Audit/Bugs.md)
