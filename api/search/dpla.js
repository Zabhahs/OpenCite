// OpenCITE — DPLA browser-shim route (v0.34) · Node runtime
// The browser app cannot hold a secret, so its DPLA adapter calls THIS same-origin
// endpoint instead of the upstream. The endpoint reads DPLA_API_KEY from env (via
...
export default async function handler(req, res) {
  ...
  try {
    const out = await DPLA_ADAPTER.search(q, serverInjectedKeys(), { offset });
