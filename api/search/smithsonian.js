// OpenCITE — Smithsonian browser-shim route (v0.34) · Node runtime
// The browser app cannot hold a secret, so its Smithsonian adapter calls THIS same-origin
// endpoint instead of the upstream. The endpoint reads SMITHSONIAN_API_KEY from env (via
...
export default async function handler(req, res) {
  ...
  try {
    const out = await SMITHSONIAN_ADAPTER.search(q, serverInjectedKeys(), { offset });
